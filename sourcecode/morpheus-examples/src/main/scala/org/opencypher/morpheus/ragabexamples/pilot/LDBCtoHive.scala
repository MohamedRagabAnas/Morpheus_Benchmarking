...
/*
    ...
    val csvFiles = getListOfFiles("/home/hadoop/social_networkldbc/")
 ...
 // Create views that normalize LDBC data where necessary
    ...
    */
      ...
      //spark.sql("select *  from ldbc.university limit 10 ").show
...
/*
    session.cypher(
      s"""
         ...
         |RETURN n.firstName, c.name, n.speaks, n.creationDate
  ...
  session.cypher(
    s"""
       |FROM GRAPH sql.LDBC
       |MATCH (f:Forum)-[r:HAS_MEMBER] -(p:Person)
       |RETURN r.joinDate
       |LIMIT 20
     """.stripMargin).show
*/
