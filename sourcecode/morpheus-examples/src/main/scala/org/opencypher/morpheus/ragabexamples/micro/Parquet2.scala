...
object Parquet2 {
  ...
  val parquetPgds = GraphSources.fs(s"hdfs://172.17.77.48:9000/user/hadoop/morpheus/$ds").parquet
  morpheus.registerSource(Namespace("myNewGraphNameSapce"), parquetPgds)
  val graph= morpheus.catalog.graph("myNewGraphNameSapce.Parquet")

  val fos = new FileOutputStream(new File(s"/home/hadoop/MorpheusBench/Expriments/Logs/micro/Parquet/$ds.txt"),true)
